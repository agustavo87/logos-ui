<div x-data="EditCreatorComponent()" class="creator-component flex h-7 items-center">
    <div x-show="!isEditing" class="creator-c-label flex-grow flex flex-row justify-between items-center">
        ...
        <div class="creator-c-control flex w-16 items-center justify-end">
            ...
            <button type="button" class="px-1 text-sm text-gray-500 hover:text-red-600 focus:outline-nonet" title="Delete" 
                x-on:click="delCreator"
            >
                <svg class="h-5 w-5 fill-current" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
        ...
        <script>
            function EditCreatorComponent() {
                return {
                    ...
                    delCreator: function () {
                        if(confirm('¿Estas seguro que deseas borrar este creador?')) {
                            this.$wire.delete();
                        }
                    }
                }
