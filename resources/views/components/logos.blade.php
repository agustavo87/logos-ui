...
let waitSelectionChange = new Promise((resolve, reject) => {
    console.log('promesa llamada')
    quill.once('selection-change', resolve);
})
...
quill.on('text-change', function(delta, oldDelta, source) {
  if (source == 'user') {
    delta.forEach((op) => {
        if (op.insert == "\n") {
            // console.log("ENTER");
            waitSelectionChange.then(corregirOffset)
        }
    });
  }
});
